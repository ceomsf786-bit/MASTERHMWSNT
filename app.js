...
function bindStaticEvents() {
  ...
  $('taskFilesInput').addEventListener('change', () => renderSelectedFilesPreview('taskFilesInput', 'taskFilesPreview'));
...
async function renderStudentTaskDetail() {
  ...
  if (!submission) {
    submissionHtml = `
      <form id="studentSubmitForm" class="grid no-print" data-task-id="${escapeHtml(task.id)}">
        ...
        <div id="studentSelectedFiles" class="preview-list"></div>
  ...
  if (cameraInput) cameraInput.addEventListener('change', renderStudentSelectedFiles);
  if (filesInput) filesInput.addEventListener('change', renderStudentSelectedFiles);
}

function renderStudentSelectedFiles() {
  const files = [...($('studentCameraInput')?.files || []), ...($('studentFilesInput')?.files || [])];
  $('studentSelectedFiles').innerHTML = files.length
    ? files.map((file) => `<div class="preview-chip"><span>${escapeHtml(file.name)}</span><span class="small muted">${formatBytes(file.size)}</span></div>`).join('')
    : '';
}
...
async function handleTaskFormSubmit(event) {
  ...
  const targetStudents = $('taskTargetStudents').value.split(',').map((email) => normaliseEmail(email)).filter(Boolean);
  const dueAt = $('taskDueAt').value ? new Date($('taskDueAt').value).toISOString() : null;
  const files = Array.from($('taskFilesInput').files || []);
...
function editTask(taskId) {
  ...
  $('taskFilesPreview').innerHTML = (task.files || []).length
    ? `<p class="small muted">Current files are already saved. Upload more files here, or delete the task to remove all files.</p>`
    : '';
...
function renderSelectedFilesPreview(inputId, outputId) {
  const files = Array.from($(inputId).files || []);
  $(outputId).innerHTML = files.length
    ? files.map((file) => `<div class="preview-chip"><span>${escapeHtml(file.name)}</span><span class="small muted">${formatBytes(file.size)}</span></div>`).join('')
    : '';
}
...
async function renderFileGrid(files, title = 'Files', printable = false) {
  ...
  for (const file of files) {
    ...
    let preview = `<div class="file-placeholder">Preview not available for this file type.<br><a href="${escapeHtml(url || '#')}" target="_blank" rel="noopener">Open file</a></div>`;
    if (url && type.startsWith('image/')) {
      preview = `<img src="${escapeHtml(url)}" alt="${escapeHtml(name)}" loading="lazy" />`;
    } else if (url && type === 'application/pdf') {
      preview = `<iframe src="${escapeHtml(url)}#view=FitH" title="${escapeHtml(name)}"></iframe>`;
    }
    cards.push(`
      <div class="file-card">
        ...
        ${url ? `<a class="btn btn-ghost btn-sm no-print" href="${escapeHtml(url)}" target="_blank" rel="noopener">Open full file</a>` : ''}
      </div>
    `);
  }
