...
function filteredStudentTasks() {
  ...
  return state.studentTasks.filter((task) => {
    ...
    const hay = `${task.title} ${task.subject || ''} ${task.instructions || ''} ${taskLinksSearchText(task.task_links)}`.toLowerCase();
...
function renderTeacherTasks() {
  ...
  const tasks = state.tasks.filter((task) => {
    ...
    const hay = `${task.title} ${task.subject || ''} ${task.instructions || ''} ${task.created_by_name || ''} ${taskLinksSearchText(task.task_links)}`.toLowerCase();
  ...
  $('teacherTasksList').innerHTML = tasks.map((task) => `
    <article class="task-card">
      ...
      <div class="meta">
        ...
        <span class="badge">Due: ${formatDateTime(task.due_at)}</span>
        <span class="badge">Total: ${escapeHtml(task.total_score)} marks</span>
        <span class="badge ${task.active ? 'badge-open' : 'badge-closed'}">${task.active ? 'Allocated to student/s' : 'Task Folder only'}</span>
      ...
      <div class="actions">
        ...
        <button class="btn btn-ghost btn-sm" type="button" data-go-allocate-task="${escapeHtml(task.id)}">Allocate</button>
...
async function handleTaskFormSubmit(event) {
  ...
  const existingTargets = getTaskTargetEmails(existingTask);
  ...
  try {
    ...
    const task = await rpc('app_teacher_upsert_task', {
      ...
      p_active: existingTargets.length > 0,
    });
...
function editTask(taskId) {
  ...
  $('taskActive').value = task.active ? 'true' : 'false';
...
function getAllocationStatus(task, student) {
  const submission = getSubmissionForTaskAndStudent(task.id, student.email);
  if (submission?.status === 'reviewed') return 'reviewed';
  if (submission?.status === 'submitted') return 'submitted';
  const allocated = taskAssignedToStudent(task, student);
  if (allocated && task.due_at && new Date(task.due_at) < new Date()) return 'overdue';
...
function renderAllocationTaskList() {
  ...
  if (!student) {
    ...
    list.innerHTML = `<div class="empty-state"><h3>Choose a student first</h3><p>Then you will see every task in the folder for that student.</p></div>`;
    return;
  }
  ...
  list.innerHTML = tasks.map((task) => {
    ...
    const submission = getSubmissionForTaskAndStudent(task.id, student.email);
    const allocated = taskAssignedToStudent(task, student);
    const canUnallocate = allocated && !submission;
    const action = !allocated && !submission
      ? `<button class="btn btn-sm" type="button" data-allocate-task="${escapeHtml(task.id)}">Allocate to this student</button>`
      : canUnallocate
        ? `<button class="btn btn-warning btn-sm" type="button" data-unallocate-task="${escapeHtml(task.id)}">Remove allocation</button>`
        : `<span class="badge badge-closed">Locked after submission</span>`;
    const scoreLine = submission?.status === 'reviewed'
      ? `<div class="small"><strong>Score:</strong> ${escapeHtml(submission.score ?? '-')} / ${escapeHtml(task.total_score)} ${submission.percent !== null && submission.percent !== undefined ? `(${escapeHtml(submission.percent)}%)` : ''}</div>`
      : '';
    return `
      <article class="task-card ${statusClass(status === 'allocated' ? 'open' : status)}" data-allocation-task-id="${escapeHtml(task.id)}">
        <h4>${escapeHtml(task.title)}</h4>
        <div class="meta">
        ...
        <p class="small muted">${escapeHtml(truncate(task.instructions, 180))}</p>
        ${scoreLine}
        <div class="actions">
          ${action}
          <button class="btn btn-ghost btn-sm" type="button" data-edit-task="${escapeHtml(task.id)}">Edit task</button>
        </div>
      </article>
    `;
  }).join('');
...
async function setTaskAllocationForSelectedStudent(taskId, allocated) {
  ...
  try {
    await rpc('app_teacher_set_task_student_allocation', {
      p_session_token: state.session.session_token,
      p_task_id: taskId,
      p_student_email: student.email,
      p_allocated: Boolean(allocated),
    });
    showToast(allocated ? 'Task allocated to student.' : 'Task allocation removed.');
    await loadTeacherData();
