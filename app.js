...
async function handleLogin(event) {
  ...
  if (mode === 'teacher') {
    await handleTeacherLogin(event, email);
    return;
  }

  await handleStudentEnter(event, email);
}

async function handleStudentEnter(event, email) {
  const stopBusy = setBusy(event.submitter, 'Checking...');

  try {
    const { data, error } = await state.client.rpc('student_can_enter', { p_email: email });
    if (error) throw error;

    const approved = Array.isArray(data) ? data[0] : data;
    if (!approved?.email) {
      $('loginMessage').textContent = 'This email is not approved yet. Ask the teacher to add it first.';
      $('loginMessage').classList.remove('hidden');
      showScreen('blockedScreen');
      return;
    }

    setStudentSession(approved.email, approved.name || null);
...
async function handleTeacherLogin(event, email) {
  const stopBusy = setBusy(event.submitter, 'Sending...');
  ...
  try {
    ...
    $('loginMessage').textContent = 'Teacher: check your email. Click the secure login link to open Teacher View.';
    $('loginMessage').classList.remove('hidden');
  } catch (error) {
    showToast(error.message, true);
  } finally {
...
function setStudentSession(email, name = null) {
  state.studentEmail = normaliseEmail(email);
  state.studentName = name;
  state.role = 'student';
  window.localStorage.setItem('snt_student_email', state.studentEmail);
  if (name) window.localStorage.setItem('snt_student_name', name);
  else window.localStorage.removeItem('snt_student_name');

  $('userBadge').textContent = state.studentName ? `${state.studentName} • ${state.studentEmail}` : state.studentEmail;
...
async function handleLogout() {
  ...
  window.localStorage.removeItem('snt_student_email');
  window.localStorage.removeItem('snt_student_name');
  state.session = null;
  state.user = null;
...
async function routeUser() {
  if (state.user) {
    ...
    try {
      const isTeacher = await getTeacherRole(email);
      if (isTeacher) {
        state.role = 'teacher';
      ...
      showToast('Teacher login is only for approved teachers. Students can use Enter Student View.', true);
      await state.client.auth.signOut();
      state.session = null;
      state.user = null;
      return;
  ...
  const savedStudentEmail = normaliseEmail(window.localStorage.getItem('snt_student_email') || '');
  const savedStudentName = window.localStorage.getItem('snt_student_name') || null;

  if (savedStudentEmail) {
    try {
      const { data, error } = await state.client.rpc('student_can_enter', { p_email: savedStudentEmail });
      if (error) throw error;
      const approved = Array.isArray(data) ? data[0] : data;
      if (approved?.email) {
        setStudentSession(approved.email, approved.name || savedStudentName);
        showScreen('studentScreen');
        await loadStudentView();
        return;
      }
    } catch (error) {
      console.warn('Student restore failed:', error.message);
    }
...
async function getTeacherRole(email) {
  ...
  return Boolean(teacherRows?.length);
}

async function loadStudentView() {
  if (!state.studentEmail) return;

  try {
    const [{ data: tasks, error: tasksError }, { data: submissions, error: submissionsError }] = await Promise.all([
      state.client.rpc('student_tasks', { p_email: state.studentEmail }),
      state.client.rpc('student_submissions', { p_email: state.studentEmail }),
    ]);
...
async function uploadStudentImages(taskId, files) {
  if (!state.studentEmail) throw new Error('Student email session missing. Please enter your email again.');
  const uploadedPaths = [];
  ...
  for (const [index, file] of files.entries()) {
    ...
    const path = `${state.studentEmail}/${taskId}/${Date.now()}-${index}-${safeFileName(file.name)}`;
...
async function handleSubmitTask(event, task) {
  ...
  try {
    ...
    const { error } = await state.client.rpc('student_upsert_submission', {
      p_email: state.studentEmail,
      p_task_id: task.id,
      p_answer: answer,
