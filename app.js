...
function localFileCardHtml(file, kind, index) {
  ...
  } else if (isPdfFile({ file_name: file.name, mime_type: type })) {
    preview = `<iframe src="${escapeHtml(url)}#view=FitH" title="${escapeHtml(file.name)}"></iframe>`;
  }
  return `
    <div class="file-card local-file-card">
      ...
      <div class="file-preview file-preview-large">${preview}</div>
    </div>
  `;
}
...
async function renderFileGrid(files, title = 'Files', printable = false) {
  ...
  for (const file of files) {
    ...
    } else if (url && isPdfFile({ file_name: name, mime_type: type, file_path: file.file_path })) {
