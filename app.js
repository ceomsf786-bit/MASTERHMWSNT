/* SNT Homework App Beta 10.7 - Supabase version with Task Folder + Links
...
const state = {
  ...
  taskLinkQueue: [],
  studentFileQueue: [],
  selectedAllocationStudentEmail: '',
  signedUrls: new Map(),
};
...
function bindStaticEvents() {
  ...
  $('addTaskLinkBtn').addEventListener('click', addTaskLinkFromInputs);
  ...
  $('allocationStudentSelect').addEventListener('change', () => {
    state.selectedAllocationStudentEmail = normaliseEmail($('allocationStudentSelect').value);
    renderAllocationTaskList();
  });
  $('allocationStatusFilter').addEventListener('change', renderAllocationTaskList);
  $('allocationSearch').addEventListener('input', renderAllocationTaskList);
  ...
  ['submissionOverviewStudent', 'submissionOverviewStatus', 'submissionOverviewSearch'].forEach((id) => {
    const el = $(id);
    if (el) el.addEventListener(id === 'submissionOverviewSearch' ? 'input' : 'change', renderSubmissionStudentOverview);
  });
...
async function loadTeacherData() {
  ...
  renderSubmissionStudentOverview();
...
function isFolderTask(task) {
  return !task?.folder_task_id;
}

function isAllocationTask(task) {
  return Boolean(task?.folder_task_id);
}

function getTaskGrade(task) {
  return task?.task_grade || extractGradeText(task?.subject || task?.title || '') || 'No grade';
}

function extractGradeText(value) {
  const match = String(value || '').match(/grade\s*\d+/i) || String(value || '').match(/gr\s*\d+/i) || String(value || '').match(/\d{1,2}/);
  if (!match) return '';
  const n = String(match[0]).match(/\d+/)?.[0];
  return n ? `Grade ${n}` : '';
}

function normalisedSubject(task) {
  return String(task?.subject || 'No subject').trim() || 'No subject';
}

function sortTasksByGradeSubject(tasks) {
  return tasks.slice().sort((a, b) => {
    const gradeDiff = getGradeNumber(getTaskGrade(b)) - getGradeNumber(getTaskGrade(a));
    if (gradeDiff !== 0) return gradeDiff;
    const subjectDiff = normalisedSubject(a).localeCompare(normalisedSubject(b));
...
function getFolderTasks() {
  return sortTasksByGradeSubject(state.tasks.filter(isFolderTask));
}

function getAllocationTasks() {
  return state.tasks.filter(isAllocationTask);
}

function getStudentAllocationCopy(folderTaskId, studentEmail) {
  const email = normaliseEmail(studentEmail);
  return state.tasks.find((task) => task.folder_task_id === folderTaskId && getTaskTargetEmails(task).includes(email)) || null;
}
...
function filteredStudentTasks() {
  ...
  return state.studentTasks.filter((task) => {
    ...
    const hay = `${task.title} ${task.task_grade || ''} ${task.subject || ''} ${task.instructions || ''} ${taskLinksSearchText(task.task_links)}`.toLowerCase();
    ...
    return `
      <article class="task-card ${active} ${statusClass(status)}" data-task-id="${escapeHtml(task.id)}">
        ...
        <div class="meta">
          ...
          <span class="badge">${escapeHtml(getTaskGrade(task))}</span>
...
async function renderStudentTaskDetail() {
  ...
  const taskLinksHtml = renderTaskLinks(task.task_links || []);
...
function renderTeacherTasks() {
  ...
  const tasks = getFolderTasks().filter((task) => {
    const mine = normaliseEmail(task.created_by_email) === normaliseEmail(state.session.email);
    const hay = `${task.title} ${task.task_grade || ''} ${task.subject || ''} ${task.instructions || ''} ${task.created_by_name || ''} ${taskLinksSearchText(task.task_links)}`.toLowerCase();
    return (owner === 'all' || mine) && (!search || hay.includes(search));
  });

  if (!tasks.length) {
    $('teacherTasksList').innerHTML = `<div class="empty-state"><h3>No folder tasks found</h3><p>Add a new task/quiz above.</p></div>`;
    return;
  }
  ...
  for (const task of tasks) {
    const group = `${getTaskGrade(task)} • ${normalisedSubject(task)}`;
    if (group !== lastGroup) {
      html.push(`<div class="folder-group-title">${escapeHtml(group)}</div>`);
      lastGroup = group;
    }
    const copies = getAllocationTasks().filter((copy) => copy.folder_task_id === task.id);
    const submittedCount = state.submissions.filter((sub) => copies.some((copy) => copy.id === sub.task_id)).length;
    html.push(`
      <article class="task-card folder-master-card">
        <h4>${escapeHtml(task.title)}</h4>
        <div class="meta">
          <span class="badge">${escapeHtml(getTaskGrade(task))}</span>
          <span class="badge">${escapeHtml(task.subject || 'No subject')}</span>
          <span class="badge">Due: ${formatDateTime(task.due_at)}</span>
          <span class="badge">Total: ${escapeHtml(task.total_score)} marks</span>
          <span class="badge badge-folder">Folder master</span>
          <span class="badge">Student copies: ${copies.length}</span>
          <span class="badge">Submissions: ${submittedCount}</span>
          <span class="badge">Links: ${(task.task_links || []).length}</span>
        ...
        <div class="small"><strong>Links:</strong> ${(task.task_links || []).length ? (task.task_links || []).map((link) => escapeHtml(link.title || link.url)).join(', ') : 'None'}</div>
        <div class="small"><strong>Files:</strong> ${(task.files || []).length ? task.files.map((file) => escapeHtml(file.file_name)).join(', ') : 'None'}</div>
        <div class="actions">
          <button class="btn btn-sm" type="button" data-edit-task="${escapeHtml(task.id)}">Edit folder task</button>
          <button class="btn btn-ghost btn-sm" type="button" data-go-allocate-task="${escapeHtml(task.id)}">Allocate copy</button>
          <button class="btn btn-danger btn-sm" type="button" data-delete-task="${escapeHtml(task.id)}">Safe delete folder task</button>
        </div>
        <p class="small muted"><strong>Safe rule:</strong> Editing this folder task will not change copies already allocated to students.</p>
      </article>
    `);
  }

  $('teacherTasksList').innerHTML = html.join('');
}
...
function renderTaskStudentPicker(selectedEmails = null) {
  ...
  picker.innerHTML = Object.keys(byGrade)
    .sort((a, b) => getGradeNumber(b) - getGradeNumber(a))
    .map((grade) => `
...
function allocationLabel(task) {
  const count = (task.target_student_emails || []).filter(Boolean).length;
  if (!count) return 'Not allocated yet';
  return `Allocated to ${count} student${count === 1 ? '' : 's'}`;
}

function getTaskTargetEmails(task) {
  return (task?.target_student_emails || []).map(normaliseEmail).filter(Boolean);
}
...
async function handleTaskFormSubmit(event) {
  ...
  const existingTask = taskId ? state.tasks.find((item) => item.id === taskId) : null;
  const isStudentCopy = Boolean(existingTask?.folder_task_id);
  const existingTargets = isStudentCopy ? getTaskTargetEmails(existingTask) : [];
  const dueAt = $('taskDueAt').value ? new Date($('taskDueAt').value).toISOString() : null;
  const files = [...state.taskFileQueue];
  const taskLinks = sanitiseTaskLinks(state.taskLinkQueue);
  ...
  try {
    ...
    const task = await rpc('app_teacher_upsert_task', {
      ...
      p_task_grade: $('taskGrade').value,
      p_subject: $('taskSubject').value,
      p_instructions: $('taskInstructions').value,
      p_due_at: dueAt,
      p_total_score: Number($('taskTotalScore').value || 0),
      p_allocation_type: 'students',
      p_target_grade: '',
      p_target_student_emails: existingTargets,
      p_task_links: taskLinks,
      p_active: isStudentCopy,
    });
    ...
    clearTaskForm();
    showToast(isStudentCopy ? 'Student copy updated. Folder task is unchanged.' : (taskId ? 'Folder task updated. Existing student copies are unchanged.' : 'Folder task created.'));
    await loadTeacherData();
  } catch (err) {
    showToast(err.message, true);
  } finally {
    setBusy(button, false, 'Save to Task Folder');
  }
}
...
function editTask(taskId) {
  ...
  if (!task) return;
  const isStudentCopy = Boolean(task.folder_task_id);
  $('taskFormTitle').textContent = isStudentCopy ? 'Edit Student Copy (Folder Task Unchanged)' : 'Edit Task Folder Master';
  $('taskIdInput').value = task.id;
  $('taskGrade').value = task.task_grade || extractGradeText(task.subject || '');
  ...
  $('taskActive').value = isStudentCopy ? 'true' : 'false';
  $('taskAllocationType').value = 'students';
  $('taskTargetGrade').value = '';
  $('taskTargetStudents').value = isStudentCopy ? (task.target_student_emails || []).join(', ') : '';
  state.taskFileQueue = [];
  state.taskLinkQueue = sanitiseTaskLinks(task.task_links || []);
  $('taskFilesInput').value = '';
  renderQueuedFilesPreview('task');
  renderTaskLinksPreview();
  ...
  if (isStudentCopy) {
    $('taskFilesPreview').insertAdjacentHTML('afterbegin', `<p class="small warn-text"><strong>You are editing one student copy only.</strong> The Task Folder master will not change.</p>`);
  }
  window.scrollTo({ top: 0, behavior: 'smooth' });
}

function clearTaskForm() {
  $('taskFormTitle').textContent = 'Create / Edit Task in Task Folder';
  $('taskForm').reset();
  $('taskIdInput').value = '';
  $('taskGrade').value = '';
...
// -----------------------------
// Teacher per-student task allocation
// -----------------------------

function sortedStudentsForAllocation() {
  return state.students
    .filter((student) => student.active)
    .slice()
    .sort((a, b) => {
      const gradeDiff = getGradeNumber(b.grade) - getGradeNumber(a.grade);
      if (gradeDiff !== 0) return gradeDiff;
...
function renderAllocationControls() {
  const select = $('allocationStudentSelect');
  if (!select) return;
  const students = sortedStudentsForAllocation();
  const current = state.selectedAllocationStudentEmail || select.value || '';
  select.innerHTML = `<option value="">Choose student</option>${students.map((student) => `
    <option value="${escapeHtml(student.email)}">${escapeHtml(student.grade || 'No grade')} — ${escapeHtml(student.name || student.email)} (${escapeHtml(student.email)})</option>
  `).join('')}`;
  if (current && students.some((student) => normaliseEmail(student.email) === normaliseEmail(current))) {
    select.value = current;
    state.selectedAllocationStudentEmail = normaliseEmail(current);
  } else {
    state.selectedAllocationStudentEmail = normaliseEmail(select.value);
  }
  renderAllocationTaskList();
}

function getAllocationStudent() {
  const email = normaliseEmail(state.selectedAllocationStudentEmail || $('allocationStudentSelect')?.value || '');
  return state.students.find((student) => normaliseEmail(student.email) === email) || null;
}

function getSubmissionForTaskAndStudent(taskId, studentEmail) {
  const email = normaliseEmail(studentEmail);
  return state.submissions.find((submission) => submission.task_id === taskId && normaliseEmail(submission.student_email) === email) || null;
}

function getAllocationStatus(folderTask, student) {
  const copy = getStudentAllocationCopy(folderTask.id, student.email);
  if (!copy) return 'available';
  const submission = getSubmissionForTaskAndStudent(copy.id, student.email);
  if (submission?.status === 'reviewed') return 'reviewed';
  if (submission?.status === 'submitted') return 'submitted';
  if (copy.due_at && new Date(copy.due_at) < new Date()) return 'overdue';
  return 'allocated';
}

function allocationStatusLabel(status) {
  if (status === 'available') return 'Not allocated';
  if (status === 'allocated') return 'Allocated / new';
  if (status === 'submitted') return 'Submitted';
  if (status === 'reviewed') return 'Reviewed / completed';
  if (status === 'overdue') return 'Overdue';
  return status;
}

function renderAllocationTaskList() {
  const list = $('allocationTaskList');
  const summary = $('allocationStudentSummary');
  if (!list || !summary) return;
  const student = getAllocationStudent();
  if (!student) {
    summary.textContent = 'Choose student';
    list.innerHTML = `<div class="empty-state"><h3>Choose a student first</h3><p>Then you will see every folder task and that student’s copies.</p></div>`;
    return;
  }

  const statusFilter = $('allocationStatusFilter').value;
  const search = $('allocationSearch').value.trim().toLowerCase();
  const folderTasks = getFolderTasks().filter((task) => {
    const status = getAllocationStatus(task, student);
    const hay = `${task.title || ''} ${task.task_grade || ''} ${task.subject || ''} ${task.instructions || ''} ${taskLinksSearchText(task.task_links)}`.toLowerCase();
    return (statusFilter === 'all' || status === statusFilter)
      && (!search || hay.includes(search));
  });

  const counts = getFolderTasks().reduce((acc, task) => {
    const status = getAllocationStatus(task, student);
    acc[status] = (acc[status] || 0) + 1;
    return acc;
  }, {});
  summary.textContent = `${student.name || student.email} • New: ${counts.allocated || 0} • Submitted: ${counts.submitted || 0} • Completed: ${counts.reviewed || 0}`;

  if (!folderTasks.length) {
    list.innerHTML = `<div class="empty-state"><h3>No tasks found</h3><p>Try another filter/search.</p></div>`;
    return;
  }

  let lastGroup = '';
  const html = [];
  for (const folderTask of folderTasks) {
    const group = `${getTaskGrade(folderTask)} • ${normalisedSubject(folderTask)}`;
    if (group !== lastGroup) {
      html.push(`<div class="folder-group-title">${escapeHtml(group)}</div>`);
      lastGroup = group;
    }
    ...
    const copy = getStudentAllocationCopy(folderTask.id, student.email);
    const status = getAllocationStatus(folderTask, student);
    const displayTask = copy || folderTask;
    const submission = copy ? getSubmissionForTaskAndStudent(copy.id, student.email) : null;
    const canUnallocate = Boolean(copy) && !submission;
    const action = !copy
      ? `<button class="btn btn-sm" type="button" data-allocate-task="${escapeHtml(folderTask.id)}">Make copy for this student</button>`
      : canUnallocate
        ? `<button class="btn btn-warning btn-sm" type="button" data-unallocate-task="${escapeHtml(copy.id)}">Remove student copy</button>`
        : `<span class="badge badge-closed">Locked after submission</span>`;
    const copyNote = copy
      ? `<span class="badge badge-copy">Student copy</span>`
      : `<span class="badge badge-folder">Folder master</span>`;
    const scoreLine = submission?.status === 'reviewed'
      ? `<div class="small"><strong>Score:</strong> ${escapeHtml(submission.score ?? '-')} / ${escapeHtml(displayTask.total_score)} ${submission.percent !== null && submission.percent !== undefined ? `(${escapeHtml(submission.percent)}%)` : ''}</div>`
      : '';
    html.push(`
      <article class="task-card ${statusClass(status === 'allocated' ? 'open' : status)}" data-allocation-task-id="${escapeHtml(folderTask.id)}">
        <h4>${escapeHtml(displayTask.title)}</h4>
        <div class="meta">
          <span class="badge ${badgeClass(status === 'allocated' ? 'open' : status)}">${escapeHtml(allocationStatusLabel(status))}</span>
          ${copyNote}
          <span class="badge">${escapeHtml(getTaskGrade(displayTask))}</span>
          <span class="badge">${escapeHtml(displayTask.subject || 'No subject')}</span>
          <span class="badge">Due: ${formatDateTime(displayTask.due_at)}</span>
          <span class="badge">Total: ${escapeHtml(displayTask.total_score)} marks</span>
        </div>
        <p class="small muted">${escapeHtml(truncate(displayTask.instructions, 180))}</p>
        ${scoreLine}
        <div class="actions">
          ${action}
          ${copy ? `<button class="btn btn-ghost btn-sm" type="button" data-edit-task="${escapeHtml(copy.id)}">Edit student copy</button>` : `<button class="btn btn-ghost btn-sm" type="button" data-edit-task="${escapeHtml(folderTask.id)}">Edit folder task</button>`}
        </div>
      </article>
    `);
  }
...
async function setTaskAllocationForSelectedStudent(taskId, allocated) {
  const student = getAllocationStudent();
  if (!student) {
  ...
  try {
    const result = await rpc('app_teacher_set_task_student_allocation', {
      p_session_token: state.session.session_token,
      p_task_id: taskId,
      p_student_email: student.email,
      p_allocated: Boolean(allocated),
    });
    if (!allocated) await removeFilesFromDeleteResult(result);
    showToast(allocated ? 'Student copy created and allocated.' : 'Student copy removed.');
    await loadTeacherData();
    showTeacherTab('allocate');
  } catch (err) {
    showToast(err.message, true);
  }
}
...
function scrollTaskIntoAllocationList(taskId) {
  setTimeout(() => {
    const card = document.querySelector(`[data-allocation-task-id="${String(taskId).replace(/"/g, '')}"]`);
...
function renderSubmissionFilters() {
  const allocatedTasks = sortTasksByGradeSubject(state.tasks.filter(isAllocationTask));
  const taskOptions = allocatedTasks.map((task) => `<option value="${escapeHtml(task.id)}">${escapeHtml(getTaskGrade(task))} • ${escapeHtml(task.subject || 'No subject')} • ${escapeHtml(task.title)}</option>`).join('');
  $('submissionTaskFilter').innerHTML = `<option value="all">All allocated tasks</option>${taskOptions}`;

  const students = sortedStudentsForAllocation();
  const studentOptions = students.map((s) => `<option value="${escapeHtml(s.email)}">${escapeHtml(s.grade || '-')} — ${escapeHtml(s.name || s.email)}</option>`).join('');
  $('submissionStudentFilter').innerHTML = `<option value="all">All students</option>${studentOptions}`;
  const overview = $('submissionOverviewStudent');
  if (overview) {
    const current = overview.value;
    overview.innerHTML = `<option value="">Choose student</option>${studentOptions}`;
    if (current && students.some((s) => normaliseEmail(s.email) === normaliseEmail(current))) overview.value = current;
  }
}
...
function filteredSubmissions() {
  ...
  return state.submissions.filter((sub) => {
    const hay = `${sub.task_title} ${sub.task_grade || ''} ${sub.subject || ''} ${sub.student_email} ${sub.student_name || ''} ${sub.answer || ''}`.toLowerCase();
...
function getStudentAllocatedTasks(studentEmail) {
  const email = normaliseEmail(studentEmail);
  return sortTasksByGradeSubject(state.tasks.filter((task) => isAllocationTask(task) && getTaskTargetEmails(task).includes(email)));
}

function getOverviewStatus(task, studentEmail) {
  const sub = getSubmissionForTaskAndStudent(task.id, studentEmail);
...
function renderSubmissionStudentOverview() {
  const out = $('submissionStudentOverview');
  if (!out) return;
  const email = normaliseEmail($('submissionOverviewStudent')?.value || '');
  if (!email) {
    out.innerHTML = `<div class="empty-state"><h3>Choose a student</h3><p>See what tasks the student has, what is new, and what is completed.</p></div>`;
    return;
  }
  const statusFilter = $('submissionOverviewStatus').value;
  const search = $('submissionOverviewSearch').value.trim().toLowerCase();
  const student = state.students.find((s) => normaliseEmail(s.email) === email);
  const tasks = getStudentAllocatedTasks(email).filter((task) => {
    const status = getOverviewStatus(task, email);
    const hay = `${task.title || ''} ${task.task_grade || ''} ${task.subject || ''}`.toLowerCase();
    return (statusFilter === 'all' || status === statusFilter) && (!search || hay.includes(search));
  });
  const counts = getStudentAllocatedTasks(email).reduce((acc, task) => {
    const status = getOverviewStatus(task, email);
    acc[status] = (acc[status] || 0) + 1;
    return acc;
  }, {});
  if (!tasks.length) {
    out.innerHTML = `<div class="empty-state"><h3>No allocated tasks found</h3><p>${escapeHtml(student?.name || email)} has no tasks matching this filter.</p></div>`;
    return;
  }
  out.innerHTML = `
    <div class="allocation-summary-strip">
      <span class="badge">${escapeHtml(student?.name || email)}</span>
      <span class="badge">New: ${counts.allocated || 0}</span>
      <span class="badge">Submitted: ${counts.submitted || 0}</span>
      <span class="badge">Completed: ${counts.reviewed || 0}</span>
      <span class="badge">Overdue: ${counts.overdue || 0}</span>
    </div>
    ${tasks.map((task) => {
      const sub = getSubmissionForTaskAndStudent(task.id, email);
      const status = getOverviewStatus(task, email);
      return `
        <article class="mini-task-row ${statusClass(status === 'allocated' ? 'open' : status)}">
          <div><strong>${escapeHtml(task.title)}</strong><br><span class="small muted">${escapeHtml(getTaskGrade(task))} • ${escapeHtml(task.subject || 'No subject')} • Due: ${formatDateTime(task.due_at)}</span></div>
          <span class="badge ${badgeClass(status === 'allocated' ? 'open' : status)}">${escapeHtml(allocationStatusLabel(status))}</span>
          <span class="badge">${sub?.status === 'reviewed' ? `${escapeHtml(sub.score ?? '-')} / ${escapeHtml(task.total_score)}` : 'No score yet'}</span>
        </article>
      `;
    }).join('')}
  `;
...
async function submissionCardHtml(sub) {
  ...
  return `
    <details class="submission-accordion panel panel-black ${sub.status === 'reviewed' ? 'status-reviewed' : 'status-submitted'}">
      <summary class="submission-summary">
        <div>
          <strong>${escapeHtml(sub.task_title)}</strong><br>
          <span class="small muted">${escapeHtml(sub.student_name || '-')} • ${escapeHtml(sub.student_email)} • ${escapeHtml(sub.student_grade || '-')}</span>
        </div>
        <div class="summary-badges">
          <span class="badge ${badgeClass(sub.status)}">${statusLabel(sub.status)}</span>
          ${closed}
          <span class="badge">Score: ${escapeHtml(sub.score ?? '-')} / ${escapeHtml(sub.total_score)}</span>
          <span class="badge">Submitted: ${formatDateTime(sub.submitted_at)}</span>
        </div>
      </summary>
      <div class="submission-body">
        <div class="btn-row no-print submission-actions-top">
...
async function openExportModal(submissionId) {
  const sub = state.submissions.find((item) => item.id === submissionId);
  if (!sub) return;
  const taskLinks = renderTaskLinks(sub.task_links || []);
...
function renderScoreFilters() {
  const grades = [...new Set(state.students.map((s) => s.grade).filter(Boolean))]
    .sort((a, b) => getGradeNumber(b) - getGradeNumber(a));
...
function taskAssignedToStudent(task, student) {
  const email = normaliseEmail(student.email);
  if (!task.active || !isAllocationTask(task)) return false;
  return getTaskTargetEmails(task).includes(email);
}
...
async function handleDynamicClick(event) {
  ...
  if (target.dataset.removeTaskLink !== undefined) { removeTaskLink(Number(target.dataset.removeTaskLink)); return; }
  if (target.dataset.editTask) { showTeacherTab('tasks'); editTask(target.dataset.editTask); }
  if (target.dataset.deleteTask) await deleteTask(target.dataset.deleteTask);
  if (target.dataset.goAllocateTask) { showTeacherTab('allocate'); renderAllocationControls(); scrollTaskIntoAllocationList(target.dataset.goAllocateTask); }
  if (target.dataset.allocateTask) await setTaskAllocationForSelectedStudent(target.dataset.allocateTask, true);
  if (target.dataset.unallocateTask) await setTaskAllocationForSelectedStudent(target.dataset.unallocateTask, false);
...
// -----------------------------
// Task links / resource links
// -----------------------------

function addTaskLinkFromInputs() {
  const titleInput = $('taskLinkTitle');
  const urlInput = $('taskLinkUrl');
  const title = titleInput.value.trim();
  const rawUrl = urlInput.value.trim();
  if (!rawUrl) {
    showToast('Add a link URL first.', true);
    return;
  }
  const url = normaliseTaskUrl(rawUrl);
  if (!isSafeHttpUrl(url)) {
    showToast('Use a valid http:// or https:// link.', true);
    return;
  }
  state.taskLinkQueue.push({ title: title || url, url });
  titleInput.value = '';
  urlInput.value = '';
  renderTaskLinksPreview();
}
...
function removeTaskLink(index) {
  ...
  state.taskLinkQueue.splice(index, 1);
  renderTaskLinksPreview();
}

function renderTaskLinksPreview() {
  const output = $('taskLinksPreview');
  if (!output) return;
  const links = sanitiseTaskLinks(state.taskLinkQueue);
  state.taskLinkQueue = links;
  if (!links.length) {
    output.innerHTML = '<p class="small muted">No links added yet.</p>';
    return;
  }
  output.innerHTML = links.map((link, index) => `
    <div class="link-chip">
      <a href="${escapeHtml(link.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(link.title || link.url)}</a>
      <button class="btn btn-danger btn-sm" type="button" data-remove-task-link="${index}">Remove</button>
    </div>
  `).join('');
}

function renderTaskLinks(links) {
  const safeLinks = sanitiseTaskLinks(links);
  if (!safeLinks.length) return '<p class="muted small">No task links.</p>';
  return `
    <section class="task-links-section">
      <h3>Task links / online resources</h3>
      <div class="task-links-list">
        ${safeLinks.map((link) => `
          <a class="task-link-card" href="${escapeHtml(link.url)}" target="_blank" rel="noopener noreferrer">
            <strong>${escapeHtml(link.title || link.url)}</strong>
            <span>${escapeHtml(link.url)}</span>
          </a>
        `).join('')}
      </div>
      <p class="small muted no-print">Links open in a new tab so this homework app stays open.</p>
    </section>
  `;
}

function sanitiseTaskLinks(links) {
  if (!Array.isArray(links)) return [];
  return links.map((link) => {
    const url = normaliseTaskUrl(link?.url || '');
    const title = String(link?.title || '').trim().slice(0, 120) || url;
    return { title, url };
  }).filter((link) => isSafeHttpUrl(link.url)).slice(0, 30);
}

function normaliseTaskUrl(url) {
...
function taskLinksSearchText(links) {
  return sanitiseTaskLinks(links).map((link) => `${link.title} ${link.url}`).join(' ');
}
